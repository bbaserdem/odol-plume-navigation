...
% DEBUG
dist_debug =   zeros( N );
uxdiff_debug = zeros( N );
uydiff_debug = zeros( N );
distw_debug =   zeros( N );
uwxdiff_debug = zeros( N );
uwydiff_debug = zeros( N );
...
for t = 1:324
    ...
    xdiff_debug = COR(:,2) - (COR(:,2)');
    ydiff_debug = COR(:,1) - (COR(:,1)');
    ...
    sto = (abs(xdiff_debug)<(KDOM*H)) & (abs(ydiff_debug)<(KDOM*H));
    dist_debug(sto) = xdiff_debug.^2 + ydiff_debug.^2;
    sto(sto) = dist_debug(sto) < ((KDOM*H).^2);
    dist_debug(sto) = sqrt(dist_debug(sto));
    dist_debug(~sto) = 0;
    ...
    FXdis = FXext + sum( ewx_ij .* sto , 2 );
    FYdis = FYext + sum( ewy_ij .* sto , 2 );
